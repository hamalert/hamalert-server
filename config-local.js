...
config.dstar = {
	...
	smartGroups: {
		...
		refreshInterval: 3600*1000,
		timeout: 30000,
		dumpFile: path.join(__dirname, '.local', 'cache', 'dstar-groups.dump'),
		// Seeds lookups before the first fetch completes, and again if the fetch ever fails while
		// the dump file was empty (a fresh install with no dump and no network yet). The seeded
		// local trigger watches DSTAR1 (see tools/seedLocalUser.js).
		static: [
